...
metadata_items <- c("Source","Date","Filename")
metadata_values <- c("EPA Air Quality System (AQS)", "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv")
# Create a data frame
metadata_df <- data.frame("Item" = metadata_items, "Value" = metadata_values)

# Use kable to create the table
knitr::kable(metadata_df, caption = "EPA Air Quality", align = "rl")
```
...
```{r Isolate Ozone data, echo=FALSE}
#install.packages("dplyr")
library(dplyr)
```
\newpage
```{r Creating_2018_Boxplot, echo=FALSE, fig.align='left', fig.height=3.1, fig.cap="Boxplot depicting the ozone levels for every month of 2018."}
# Load ggplot2
# install.packages("ggplot2")
library(ggplot2)

# Create a boxplot for Ozone levels in 2018
ozone2018_plot <- ggplot(subset(epa_data, Year == "2018"), 
                         aes(x = month.abb[month(Date)], y = Ozone)) +
  geom_boxplot(na.rm = TRUE) + scale_x_discrete(limits = month.abb) +
  labs(
    title = "Ozone levels in 2018",
    x = "Month",
    y = "Ozone Units"
  )

# Print the plot
print(ozone2018_plot)
```
```{r Creating_2019_Boxplot, echo=FALSE, fig.align='left', fig.height=3.1, fig.cap="Boxplot depicting the ozone levels for every month of 2019."}
# Create a boxplot for Ozone levels in 2019
ozone2019_plot <- ggplot(subset(epa_data, Year == "2019"), 
                         aes(x = month.abb[month(Date)], y = Ozone)) +
  geom_boxplot(na.rm = TRUE) + scale_x_discrete(limits = month.abb) +
  labs(
    title = "Ozone levels in 2019",
    x = "Month",
    y = "Ozone Units"
  )

# Print the plot
print(ozone2019_plot)
```
...
https://github.com/dmliddle/EDE_Fall2023
