...
library(ggplot2)
# Isolate year 2018 from data
ozone2018_data <- epa_data %>%
  select(c(Ozone, Date)) %>%
  filter(year(Date) == 2018)
#Isolate year 2019 from data
ozone2019_data <- epa_data %>%
  select(c(Ozone, Date)) %>%
  filter(year(Date) == 2019)
```

```{Create 2018 Boxplots}

ozone2018_boxplot <- ggplot(ozone2018_data, 
aes(x = month.abb[month], y = Ozone)) +
geom_boxplot(na.rm = TRUE) + scale_x_discrete(limits = month.abb) + 
   labs(title = "Ozone Levels in 2018", #Create title and label axes
       x = "Month",
       y = "Ozone Units")
